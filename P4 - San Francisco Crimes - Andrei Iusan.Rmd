...
```{r setup, echo=FALSE, warnings=FALSE, message=FALSE, warning=FALSE}
library(ggplot2)
library(ggmap)
library(dplyr)
library(tidyr)
...
```{r Structure, echo=FALSE, warnings=FALSE}
str(crimes)
```
...
```{r preprocessing, echo=FALSE, warnings=FALSE}
...
```{r New Structure, echo=FALSE, warnings=FALSE}
str(crimes)
```
...
```{r summary, echo=FALSE, warnings=FALSE}
summary(crimes)
```
...
```{r Reorder factors, echo=FALSE, warnings=FALSE}
...
```{r Crimes Categories, echo=FALSE, warnings=FALSE}
...
specific crimes are *Assault*, *Vehicle Theft* and *Drug/Narcotic*.

## Resolution

I'll explore the distribution of resolutions:

```{r Crime Resolutions, echo=FALSE, warnings=FALSE}
...
```{r Crime Resolutions logarithmic, echo=FALSE, warnings=FALSE}
ggplot(data = crimes, aes(x = Resolution)) +
  geom_bar() +
  coord_flip() +
  scale_y_log10() +
...
```{r Crimes Day of Week, echo=FALSE, warnings=FALSE}
...
```{r Crimes District, echo=FALSE, warnings=FALSE}
...
```{r All Crimes Over Time, echo=FALSE, warnings=FALSE}
...
```{r All Crimes Yearly Pattern, echo=FALSE, warnings=FALSE}
...
```{r Houely Crimes, echo=FALSE, warnings=FALSE}
...
```{r Addresses frequency and map, echo=FALSE, warnings=FALSE}
...
```{r Map of Most dangerous addresses, echo=FALSE, warnings=FALSE}
...
```{r Districts on map, echo=FALSE, warnings=FALSE}
ggmap(SFMap) +
...
```{r Descriptions plot, echo=FALSE, warnings=FALSE}
...
```{r resolution / no resolution, echo=FALSE, warnings=FALSE}
...
```{r Crimes vs percent of solved, echo=FALSE, warnings=FALSE}
...
```{r Category vs Resolutions heatmap, echo=FALSE, warnings=FALSE}
...
```{r Category vs District, echo=FALSE, warnings=FALSE}
...
```{r Categories time, echo=FALSE, warnings=FALSE}
...
```{r Crimes by District Over Time, echo=FALSE, warnings=FALSE}
categ_time <- crimes[top_categories(6),]
categ_time$DateTime <- as.Date(round(categ_time$DateTime, units = "days"))
categ_time <- categ_time %>%
  mutate(year = year(DateTime),
...
```{r Density of Most Common Crimes, echo=FALSE, warnings=FALSE}
...
It seems that vehicles are stolen from anywhere in the city. For drugs on the
other hand, it looks like there are a few large centers.

```{r Drugs heatmap, echo=FALSE, warnings=FALSE}
ggmap(SFMap) +
  stat_density2d(
    aes(x = X, y = Y, fill = ..level..),
    n = 200,
...
```{r Drugs points, echo=FALSE, warnings=FALSE}
ggmap(SFMap) +
  geom_point(
    aes(x = X, y = Y),
    data = filter(crimes, Category == "DRUG/NARCOTIC"),
...
```{r Drugs Yearly heatmap, echo=FALSE, warnings=FALSE}
...
```{r Resolution Percentage Time, echo=FALSE, warnings=FALSE}
...
```{r Final Plot 1, echo=FALSE, warnings=FALSE}
...
I think this simple plot tells us a lot. We can instantly see which crimes are
more likely to be resolved, and at the same time see what crimes are most
frequent. I think this color gradient is good because it makes a clear split 
at 50%. If the bar is slightly red, we know more crimes are unsolved in that 
category, if it's slightly blue, we have more of them solved.

## Second Plot

```{r Final Plot 2, echo=FALSE, warnings=FALSE}
ggplot(data = resolutions_month_district,
       aes(x = dt, y = 100 * yes / (yes + no))) +
...
This plot is also simple, yet clear. It shows us the difference between amount
of crimes that had resolution in different districts, over time.

## Third plot

```{r Final Plot 3, echo=FALSE, warnings=FALSE}
...
Tenderloin, so we can have a look at crimes activity at street level.
Interesting fact, at the bottom-right corner we can see the address of 
[SFPOA](http://www.sfpoa.org).
...
variable, I had to do some more preprocessing. I feel that ggplot's style of
sending the whole data to the plotting library and selecting what to plot and
how is useful most of the time, because it allows you to quickly create
relatively complex plots, but at other times it's confusing because I'm not
sure if a plot can be created with ggplot directly or I need to process the
data first. But I assume this gets easier after more practice. The approach
I had until now was to prepare the data first (for example using matplotlib
in python).
...
by using a similar approach I used for Drug crimes. This analysis shows 
a mile high overview of the data, but there is definetly a lot more hidden
